...
OUT_DIR = Path("/home/songxy/code/Diffusion-Ergodic/diffusion_ergodic_results/toes_continuation_groupcomplete_v7c_ergodic_margin")
...
def main():
    ...
    config_dict["v7c_conditioning_fixes"] = [
    ...
    if wandb is not None:
        run = wandb.init(
            ...
            name="toes_continuation_groupcomplete_v7c_ergodic_margin",
            config=config_dict,
            reinit=True,
            mode=os.environ.get("WANDB_MODE", "online"),
        )

    print("[v7c preflight] data_dir:", DATA_DIR)
    print("[v7c preflight] stats_data_dir:", STATS_DATA_DIR)
    print("[v7c preflight] dataset size:", len(pairs))
    print("[v7c preflight] gamma counts:", dict(sorted(gamma_counts.items())))
    print("[v7c preflight] complete 4-gamma distributions:", len(complete_dists))
    print("[v7c preflight] train distributions:", len(train_set), "val distributions:", len(val_set), "test distributions:", len(test_set))
    print("[v7c preflight] complete train groups:", len(train_sampler.groups), "steps/epoch:", len(train_loader))
    print("[v7c preflight] complete val trajectories:", len(val_idx), "complete val groups:", len(val_idx) // 4)
    print("[v7c preflight] split mode: distribution group split; gamma variants stay in the same split")
    print("[v7c preflight] stats robot_state mean:", stats["robot_state"]["mean"].tolist())
    print("[v7c preflight] init checkpoint:", INIT_CKPT)
    print("[v7c preflight] output dir:", OUT_DIR)
    print("[v7c preflight] wandb mode:", os.environ.get("WANDB_MODE", "online"))
    print("[v7c preflight] endpoint conditioning: start index 0, end index 100")
    print("[v7c preflight] loss weights:", {
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        print(
            f"[v7c epoch {epoch}] loss={total_loss/max(n_steps,1):.4f} mse={total_mse/max(n_steps,1):.4f} "
        ...
        if erg_val < best_ergodic:
            ...
            print(f"[v7c] saved best ergodic: {best_ergodic:.4f}")
        if mixed < best_mixed:
            best_mixed = mixed
            no_improve = 0
            improved = True
            save_checkpoint(OUT_DIR / "best_mixed_model.pth", model, opt, epoch, metrics, config_dict, gm, score=mixed)
            print(f"[v7c] saved best mixed: {best_mixed:.4f}")
        if not improved:
            no_improve += 1
        if epoch % 20 == 0:
            torch.save({"epoch": epoch, "model_state_dict": model.state_dict(), "config": config_dict}, OUT_DIR / f"checkpoint_epoch_{epoch}.pth")
        if no_improve >= PATIENCE:
            print(f"[v7c] early stop at epoch {epoch}; best_ade={best_ade:.4f} best_morph={best_morph:.4f} best_ergodic={best_ergodic:.4f} best_mixed={best_mixed:.4f}")
            break
    ...
    with open(OUT_DIR / "v7c_training_and_eval_summary.json", "w") as f:
        json.dump(jsonable(final_summary), f, indent=2)
    if wandb is not None:
        wandb.finish()
    print("[v7c] summary:", OUT_DIR / "v7c_training_and_eval_summary.json")
