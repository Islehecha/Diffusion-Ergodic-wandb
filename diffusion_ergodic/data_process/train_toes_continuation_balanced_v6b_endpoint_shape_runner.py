...
DATA_DIR = "/home/songxy/code/Diffusion-Ergodic/diffusion_ergodic/data/ergodic_toes_continuation_v1"
STATS_DATA_DIR = "/home/songxy/code/Diffusion-Ergodic/diffusion_ergodic/data/ergodic_toes_continuation_v1_balanced"
OUT_DIR = Path("/home/songxy/code/Diffusion-Ergodic/diffusion_ergodic_results/toes_continuation_balanced_v6b_endpoint_shape_groupcomplete")
INIT_CKPT = "/home/songxy/code/Diffusion-Ergodic/diffusion_ergodic_results/toes_continuation_balanced_v5_timestep_film/best_model.pth"
...
INFERENCE_STEPS = 50
T_FIXED = 0.5
SHAPE_LOSS_WEIGHT = 0.20
MIXED_SCORE_WEIGHT = 0.75
MAX_EPOCHS = 180
PATIENCE = 28
BATCH_DISTS = 12
...
def split_distribution_sets(raw_ds, seed=42, val_split=0.1, test_split=0.05):
    all_dist_files = sorted(set(p["distribution_file"] for p in raw_ds.data_pairs))
    ...
    val_set = set(all_dist_files[n_test:n_test + n_val])
    train_set = set(all_dist_files[n_test + n_val:])
...
def gamma_shape_loss(pred_norm, gt_norm, stats, groups_per_batch):
    ...
    for i, j in itertools.combinations(range(4), 2):
        ...
        losses.append(torch.abs(d_pred - d_gt))
...
def main():
    ...
    assert len(pairs) == 2722, len(pairs)
    assert dict(sorted(gamma_counts.items())) == {"0.010000": 339, "0.030000": 444, "0.100000": 949, "0.200000": 990}
    ...
    config_dict["v6_conditioning_fixes"] = [
        "start_and_endpoint_conditioning",
        "per_timestep_decoder_tokens",
        "block_level_gamma_film",
        "grouped_gamma_batches",
        "oracle_pairwise_shape_loss",
        "best_ade_best_morphology_best_mixed_checkpoints",
    ]
    config_dict["shape_loss_weight"] = SHAPE_LOSS_WEIGHT
    config_dict["mixed_score_weight"] = MIXED_SCORE_WEIGHT
    ...
    if wandb is not None:
        run = wandb.init(
            ...
            name="toes_continuation_balanced_v6b_endpoint_shape_groupcomplete",
            config=config_dict,
            reinit=True,
            mode="online",
        )

    print("[v6 preflight] data_dir:", DATA_DIR)
    print("[v6 preflight] stats_data_dir:", STATS_DATA_DIR)
    print("[v6 preflight] dataset size:", len(pairs))
    print("[v6 preflight] gamma counts:", dict(sorted(gamma_counts.items())))
    print("[v6 preflight] train groups:", len(train_sampler.groups), "steps/epoch:", len(train_loader))
    print("[v6 preflight] val idx:", len(val_idx))
    print("[v6 preflight] stats robot_state mean:", stats["robot_state"]["mean"].tolist())
    print("[v6 preflight] init checkpoint:", INIT_CKPT)
    print("[v6 preflight] output dir:", OUT_DIR)
    print("[v6 preflight] endpoint conditioning: start index 0, end index 100")
    print("[v6 preflight] shape loss weight:", SHAPE_LOSS_WEIGHT)
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        model.train()
        total_mse = total_shape = total_loss = 0.0
        n_steps = 0
        for batch in train_loader:
            ...
            loss = mse + SHAPE_LOSS_WEIGHT * shape + 0.05 * endp
            loss.backward()
        ...
        mixed = ade + MIXED_SCORE_WEIGHT * morph
        print(
            f"[v6 epoch {epoch}] loss={total_loss/max(n_steps,1):.4f} mse={total_mse/max(n_steps,1):.4f} "
            f"shape={total_shape/max(n_steps,1):.4f} direct_ade={direct_ade:.4f} dpm_ade={ade:.4f} "
            f"morph_err={morph:.4f} pred_pair={gm['direct_pred_pairwise_mean']:.4f} "
            f"oracle_pair={gm['oracle_pairwise_mean']:.4f} mixed={mixed:.4f}"
        )
    ...
    final_summary = {
        ...
        "mixed_score_weight": MIXED_SCORE_WEIGHT,
        "checkpoints": {},
    }
    for label, ckpt in {
    ...
    with open(OUT_DIR / "v6_training_and_eval_summary.json", "w") as f:
        json.dump(jsonable(final_summary), f, indent=2)
    if wandb is not None:
        wandb.finish()
    print("[v6] summary:", OUT_DIR / "v6_training_and_eval_summary.json")
