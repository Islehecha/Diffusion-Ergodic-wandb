...
DATA_DIR = "/home/songxy/code/Diffusion-Ergodic/diffusion_ergodic/data/ergodic_toes_continuation_v1_groupcomplete_balanced"
STATS_DATA_DIR = DATA_DIR
OUT_DIR = Path("/home/songxy/code/Diffusion-Ergodic/diffusion_ergodic_results/toes_continuation_groupcomplete_v8_coverage_budget")
...
def pack_distribution_np(dist_data, max_gaussians=20):
...
def hotspot_coverage_score_np(xy, packed, valid, temperature=12.0):
    centers = packed[:, 0:2]
    cov_flat = packed[:, 2:6]
    weights = packed[:, 6]
...
def gamma_key(gamma, gammas=GAMMAS, tol=1e-6):
...
class ErgodicDataset(Dataset):
    def __init__(self, data_dir, transform=None, max_trajectory_len=101, use_index=True, max_gaussians=20):
        ...
        self.coverage_budget_by_dist_gamma = {}
        self.coverage_budget_by_pair = self._build_coverage_budget_cache()
    ...
    def _build_coverage_budget_cache(self):
        grouped = collections.defaultdict(list)
        for pair in self.data_pairs:
            grouped[pair["distribution_file"]].append(pair)
        cache = {}
        for dfile, pairs in grouped.items():
            dist_data = json.load(open(os.path.join(self.distributions_dir, dfile)))
            packed, valid = pack_distribution_np(dist_data, max_gaussians=self.max_gaussians)
            cov_by_gamma = {}
            traj_by_gamma = {}
            for pair in pairs:
                traj_data = json.load(open(os.path.join(self.trajectories_dir, pair["trajectory_file"])))
                g = gamma_key(traj_data["gamma"])
                if g is None:
                    continue
                xy = np.asarray(traj_data["states"], dtype=np.float64)[:, :2]
                cov_by_gamma[g] = hotspot_coverage_score_np(xy, packed, valid)
                traj_by_gamma[g] = pair["trajectory_file"]
            if all(g in cov_by_gamma for g in GAMMAS):
                low = cov_by_gamma[min(GAMMAS)]
                high = cov_by_gamma[max(GAMMAS)]
                denom = max(low - high, 1e-6)
                for g in GAMMAS:
                    budget = np.clip((cov_by_gamma[g] - high) / denom, 0.0, 1.0)
                    cache[(dfile, traj_by_gamma[g])] = {
                        "coverage_budget": float(budget),
                        "oracle_coverage_score": float(cov_by_gamma[g]),
                    }
                    self.coverage_budget_by_dist_gamma[(dfile, float(g))] = float(budget)
            else:
                for pair in pairs:
                    traj_data = json.load(open(os.path.join(self.trajectories_dir, pair["trajectory_file"])))
                    g = float(traj_data["gamma"])
                    budget = float(np.clip((math.log(max(g, 1e-6)) - math.log(0.2)) / (math.log(0.01) - math.log(0.2)), 0.0, 1.0))
                    cache[(dfile, pair["trajectory_file"])] = {
                        "coverage_budget": budget,
                        "oracle_coverage_score": float("nan"),
                    }
                    self.coverage_budget_by_dist_gamma[(dfile, g)] = budget
        return cache
    ...
    def __getitem__(self, idx):
        ...
        budget_meta = self.coverage_budget_by_pair.get((pair["distribution_file"], pair["trajectory_file"]), {"coverage_budget": float(gamma), "oracle_coverage_score": float("nan")})
        ...
        sample = {
            ...
            "gamma": as_tensor_f32(budget_meta["coverage_budget"]).view(1),
            "raw_gamma": as_tensor_f32(gamma).view(1),
            "oracle_coverage_score": as_tensor_f32(budget_meta["oracle_coverage_score"]).view(1),
...
def make_model_input(raw_ds, standardizer, pair_idx, gamma, device):
    ...
    pair = raw_ds.data_pairs[pair_idx]
    budget = raw_ds.coverage_budget_by_dist_gamma.get((pair["distribution_file"], float(gamma)))
    if budget is None:
        budget = float(np.clip((math.log(max(float(gamma), 1e-6)) - math.log(0.2)) / (math.log(0.01) - math.log(0.2)), 0.0, 1.0))
    sample["gamma"] = torch.tensor([budget], dtype=torch.float32)
    return {
...
def main():
    ...
    train_loader, train_sampler = build_grouped_train_loader(raw_ds, standardizer, train_set)
    ...
    assert len(pairs) == 628, len(pairs)
    assert dict(sorted(gamma_counts.items())) == {"0.010000": 157, "0.030000": 157, "0.100000": 157, "0.200000": 157}
    assert len(complete_dists) == 157, len(complete_dists)
    ...
    config_dict["conditioning_scalar"] = "coverage_budget_0_high_gamma_to_1_low_gamma"
    ...
    if wandb is not None:
        run = wandb.init(
            ...
            name="toes_continuation_groupcomplete_v8_coverage_budget",
            config=config_dict,
            reinit=True,
            mode=os.environ.get("WANDB_MODE", "online"),
        )

    print("[v8 preflight] data_dir:", DATA_DIR)
    print("[v8 preflight] stats_data_dir:", STATS_DATA_DIR)
    print("[v8 preflight] dataset size:", len(pairs))
    print("[v8 preflight] gamma counts:", dict(sorted(gamma_counts.items())))
    print("[v8 preflight] complete 4-gamma distributions:", len(complete_dists))
    print("[v8 preflight] train distributions:", len(train_set), "val distributions:", len(val_set), "test distributions:", len(test_set))
    print("[v8 preflight] complete train groups:", len(train_sampler.groups), "steps/epoch:", len(train_loader))
    print("[v8 preflight] complete val trajectories:", len(val_idx), "complete val groups:", len(val_idx) // 4)
    print("[v8 preflight] split mode: distribution group split; gamma variants stay in the same split")
    print("[v8 preflight] stats robot_state mean:", stats["robot_state"]["mean"].tolist())
    print("[v8 preflight] init checkpoint:", INIT_CKPT)
    print("[v8 preflight] output dir:", OUT_DIR)
    print("[v8 preflight] wandb mode:", os.environ.get("WANDB_MODE", "online"))
    print("[v8 preflight] endpoint conditioning: start index 0, end index 100")
    print("[v8 preflight] conditioning scalar: normalized oracle coverage budget, not raw gamma")
    print("[v8 preflight] loss weights:", {
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        for batch in train_loader:
            ...
            shape = gamma_shape_loss(pred, X["trajectories"], stats, pred.shape[0] // 4)
        ...
        print(
            f"[v8 epoch {epoch}] loss={total_loss/max(n_steps,1):.4f} mse={total_mse/max(n_steps,1):.4f} "
        ...
        if erg_val < best_ergodic:
            ...
            print(f"[v8] saved best ergodic: {best_ergodic:.4f}")
        if mixed < best_mixed:
            best_mixed = mixed
            no_improve = 0
            improved = True
            save_checkpoint(OUT_DIR / "best_mixed_model.pth", model, opt, epoch, metrics, config_dict, gm, score=mixed)
            print(f"[v8] saved best mixed: {best_mixed:.4f}")
        if not improved:
            no_improve += 1
        if epoch % 20 == 0:
            torch.save({"epoch": epoch, "model_state_dict": model.state_dict(), "config": config_dict}, OUT_DIR / f"checkpoint_epoch_{epoch}.pth")
        if no_improve >= PATIENCE:
            print(f"[v8] early stop at epoch {epoch}; best_ade={best_ade:.4f} best_morph={best_morph:.4f} best_ergodic={best_ergodic:.4f} best_mixed={best_mixed:.4f}")
            break
    ...
    with open(OUT_DIR / "v8_training_and_eval_summary.json", "w") as f:
        json.dump(jsonable(final_summary), f, indent=2)
    if wandb is not None:
        wandb.finish()
    print("[v8] summary:", OUT_DIR / "v8_training_and_eval_summary.json")
